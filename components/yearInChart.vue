<template>
  <view>
    <view class="chart_title">
      <text class="chart_title_tip">共收入{{ notesCount }}笔，合计</text>
      <text class="chart_title_money"> ￥{{ priceTotal }} </text>
    </view>
    <text class="expenses">收入对比</text>
    ...
    <text class="expenses">收入排行榜</text>
    <view class="card-list">
      ...
      >
        <uni-icons
          customPrefix="t-icon"
          color="#ff9799"
          :type="handleIcons(item.icon_type).name"
          size="30"
        >
        </uni-icons>
...
<script>
import { notesAnalyse, notesYear } from '@/api/editor.js'
  ...
  methods: {
    ...
    async getMonthData() {
      try {
        ...
        let beginTime = moment(time).startOf('year').valueOf('x')
        let endTime = moment(time).endOf('year').valueOf('x')
        let parmas = {
          incOrExp: 1,
          beginTime,
          endTime,
        }
        let result = await notesYear(parmas)
        if (result.status === 200) {
          ...
          result.data.forEach((item) => {
            let index = item.month.split('-')[1] * 1 - 1
            monthSeriesData[index] = Math.round(item.monthTotal * 100) / 100
          })
          this.chartData.categories = months
          this.chartData.series[0].data = monthSeriesData
        }
      } catch (e) {
        console.log(e, '.')
      }
      ...
      try {
        ...
        if (result.status == 200) {
          ...
          this.list = result.data.list.map((data) => {
            ...
            data.name = this.handleIcons(data.icon_type).title
