<template>
  <view class="content" @click="onClose">
    ...
    <view class="editor__title">
      <text :class="{ active: incOrExp == 0 }" @click="selectType(0)"
        >支出</text
      >
      <text :class="{ active: incOrExp == 1 }" @click="selectType(1)"
        >收入</text
      >
    </view>
    <!-- iconfont -->
    <swiper
      :indicator-dots="true"
      indicator-active-color="#a3a3a3"
      style="height: 260px; background-color: #fff"
    >
      <swiper-item>
        <view class="editor__icon">
          <view
            class="editor__icon--radio"
            v-for="(val, index) in iconsObject.icon"
            :key="index"
            @click="selectIcon(val.id)"
          ...
          >
            ...
            <text
              class="t-icon"
              :class="[{ active: val.id == iconType }, val.name]"
            ></text>
            <view>{{ val.title }}</view>
          </view>
        </view>
      ...
      <swiper-item>
        <view class="editor__icon">
          <view
            class="editor__icon--radio"
            v-for="(val, index) in iconsObject.lineIcon"
            :key="index"
            @click="selectIcon(val.id)"
            :class="{ active: val.id == iconType }"
          >
            ...
            <text
              class="iconfont"
              style="font-size: 33px; color: #ff9799"
              :class="[{ active: val.id == iconType }, val.name]"
            ></text>
            <view>{{ val.title }}</view>
          </view>
    ...
    <view class="editor__remark">
      ...
      ></textarea>
      <view
        v-for="(val, index) in tempFilePaths"
        :key="index"
        class="editor__remark_extra"
      >
...
import { editorToAdd, getNotesList } from '../../api/editor.js'
