    ...
    <CustomTitle>
      <template v-slot:right>
        ...
        <uni-icons
          @click="showCalendar"
          customPrefix="iconfont"
          type="icon-yonghuzhongxinicon09"
          size="22"
          color="#515151"
          style="margin-right: 20rpx"
        ></uni-icons>
    ...
    <view class="content-pad">
      <view class="year" @click="toLink">
        ...
        <view class="flex text--large">
          <view>
            ...
            <text class="year_money">{{ userInfo.deposit_money }}</text>
          </view>
          <view class="year-btn">
      ...
      <view class="current">
        ...
        <view @click="setCurrentPrice">
          <text v-if="accountBookInfo.budget" class="text--title"
            >预算额度: ￥{{ accountBookInfo.budget }}</text
          >
      ...
      <view class="next" v-for="(val, index) in notesList" :key="index">
        ...
        <uni-swipe-action>
          <uni-swipe-action-item
            ...
            v-for="val in val.list"
            :key="val.id"
            style="border-top: 1px solid #eee"
            @click="onClickRemove(val)"
          >
            <view class="next__item" @click="onClickDetail(val)">
              <view class="next__item--fix">
                <uni-icons
                  customPrefix="t-icon iconfont"
                  color="#ff9799"
                  :type="handleIcons(val.icon_type).name"
                  size="30"
                >
                </uni-icons>
              </view>
              ...
              <view class="next__item--right">
                <view class="next__right--text">
                  <text style="font-size: 32rpx">{{
                    handleIcons(val.icon_type).title
                  }}</text>
                  <view style="color: rgba(138, 138, 138, 1)">{{
                    val.REMARK
                  }}</view>
                </view>
...
import { accountBook } from '../api/accountBook.js'
      ...
      try {
        ...
        if (result.status == 200) {
          ...
          arr.forEach(async (item) => {
            let date = moment(item.update_date).format('YYYY-MM-DD')
